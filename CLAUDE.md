...
**Key modules:**
- `orchestrator/workflow.py` — LangGraph `@entrypoint` (`WORKFLOW_VERSION` 2.0.0). The git rails `verify-clean-tree → branch → … → commit → push → open-pr` implicitly wrap the pipeline; the **post-branch stages run in `flow` order** via `_dispatch_stage`, reading `config.pipeline` (Phase 68b — replaced the v1 hard-coded spine + `[[steps.work]]` seam). plan + decompose run *before* the branch so the approval shows the task list. The built-in **`task-build` station (Phase 56)** runs the decomposed task list one produce⇄gate `build` per task (`_run_build_step` / `run_retry_block`); a whole-diff `qa` stage can run once after. Phase 15 split commit/push/PR into three idempotent `@task`s.
...
- `orchestrator/config.py` — loads the v2 `orchestrator.toml` (`flow` + `[stage.*]` / `[builtin.*]` / `[defs.*]`) into a typed `OrchestratorConfig` carrying a resolved `pipeline`; a v1 `[workflow.*]` / `[[steps.work]]` file is rejected with a migration message. All fields optional.
- `orchestrator/pipeline.py` — the v2 schema: parses/validates the `flow` + stage/part tables into a resolved, flow-ordered `Pipeline` (`build_pipeline`); `flow.py` is the Airflow-style flow parser; `migrate.py` converts a v1 config to v2.
...
**Config file:** `orchestrator.toml` at the project root — the **v2 declarative pipeline** (fully annotated in `orchestrator.example.toml`). A single `flow` line orders the stages (Airflow-style: `>>` sequential, `[a, b]` parallel); the git rails are implicit and locked. Two axes place everything: in-the-flow **stages** (`[stage.builtin.<id>]` for the built-ins plan/decompose/task-build/docs/summarize/qa; `[stage.user.<id>]` for yours, `type` = `script`|`ai_agent`|`build`) vs reusable **parts** referenced by a stage's `produce`/`gate` (`[builtin.<id>]` → `builtin:<id>`, `[defs.<id>]` → `defs:<id>`). A built-in named in `flow` with no table is auto-supplied with its default; a built-in stage/part's `path` overrides its prompt, and a `.orchestrator/prompts/<name>.md` frontmatter supplies its model/tools (an explicit TOML value wins). **The per-task recipe lives on the `task-build` stage** (`produce`/`gate` refs; `retry = { max, on_exhausted }`, default `approval_gate` = pause-and-ask; per-task `human_in_loop = { after_producer, on_gate_fail }`). QA gates each task by default — point `gate` at a cheap script and add a whole-diff `qa` stage for the cost-shaped pattern. Infra/rail tables: `[branch]` (+ `human_in_loop` pre-branch pause), `[pr]` (+ `human_in_loop` pre-PR pause), `[git]`, `[pre_hooks]`, `[qa]` (scripted gate), `[audit]`. Resume is gated on `WORKFLOW_VERSION` + a hash of the resolved pipeline (a mid-run config edit refuses the resume). **`fully_autonomous`** (env `ORCHESTRATOR_FULLY_AUTONOMOUS`) makes a run never pause — every `human_in_loop` gate is suppressed and build retries become unbounded; rails `autonomous_max_seconds` / `autonomous_max_cost_usd` (both `<= 0` = off, autonomous-only) trip the cancel path → `status="cancelled"`, `reason="autonomous_ceiling"`; otherwise `cancel_run` is the only stop.
